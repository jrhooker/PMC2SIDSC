<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:template match="/">
        <xsl:variable name="registers" select="//spirit:register"/>
        <xsl:result-document href="registers.xml">
            <topic>
                <title>Registers</title>
                <body>
                    <xsl:for-each select="$registers">
                        <xsl:call-template name="register-conversion"/>
                    </xsl:for-each>
                </body>
            </topic>
        </xsl:result-document>
    ...
    <xsl:template name="register-conversion">
        ...
        <xsl:element name="table">
            ...
            <xsl:element name="tgroup">
                ...
                <colspec colname="A" colwidth="111*"/>                
                <colspec colname="B" colwidth="144*"/>                
                <colspec colname="C" colwidth="101*"/>                
                <colspec colname="D" colwidth="87*"/>                
                <colspec colname="E" colwidth="557*"/>
            <xsl:element name="thead">
                <row>
                    <entry><p>Bit Field</p></entry>                    
                    <entry><p>Field Name</p></entry>                    
                    <entry><p>Type</p></entry>                    
                    <entry><p>Default Value</p></entry>                    
                    <entry><p>Description</p></entry>
                </row>
            </xsl:element>
                <xsl:element name="tbody">
                    <xsl:call-template name="registers"></xsl:call-template>
                </xsl:element>            
            </xsl:element>
        </xsl:element>
    </xsl:template>
    ...
    <xsl:template name="registers">
        <xsl:for-each select="spirit:field">
            <xsl:element name="field">
                <xsl:element name="field-bits">
                    <xsl:call-template name="bit-width"></xsl:call-template>
                </xsl:element>
            </xsl:element>
            ...
            <xsl:element name="field-type">
                <xsl:choose>
                    <xsl:when test="spirit:access = 'read-write'">
                        RW
                    </xsl:when>
                    <xsl:when test="spirit:access = 'read-only'">
                        RO
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="spirit:access"/>
                    </xsl:otherwise>
                </xsl:choose>   
            </xsl:element>
            ...
            <xsl:element name="field-desciption">
                <xsl:element name="p">
                    <xsl:value-of select="spirit:description"/>
                </xsl:element>
            </xsl:element>
        </xsl:for-each>
    </xsl:template>
    
    <xsl:template name="bit-width">
        <xsl:choose>
            <xsl:when test="spirit:bit-width = 1">
                <xsl:element name="single">
                    <xsl:value-of select="number(spirit:bitOffset) + number(spirit:bitWidth) - 1"/>
                </xsl:element>
            </xsl:when>
            <xsl:otherwise>
                ...
                <xsl:element name="double">
                    <xsl:element name="lsb">
                        <xsl:value-of select="number(spirit:bitOffset)"/>
                    </xsl:element>                  
                </xsl:element>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template name="topic_title">
        <xsl:element name="title">
            <xsl:value-of select="spirit:name"/> : <xsl:value-of select="spirit:description"/>
        </xsl:element>
    </xsl:template>
...
-->
    ...
    <xsl:template name="topic_attribute_manager">
        <xsl:for-each select="@*">
            <xsl:choose>
                <xsl:when test="name(.) = 'audience'"> </xsl:when>
                <xsl:when test="name(.) = 'arch'"> </xsl:when>
                <xsl:when test="name(.) = 'document'"> </xsl:when>
                <xsl:when test="name(.) = 'role'"> </xsl:when>
            </xsl:choose>
        </xsl:for-each>
        <xsl:if test="@xml:id">
            <xsl:attribute name="id">
